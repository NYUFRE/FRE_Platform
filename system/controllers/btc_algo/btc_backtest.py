...
def btc_backtest_service(request: flask.request, global_param_list: dict):
    ...
    if global_param_list["btc_data"] is None:
        message = "Please build the algorithm first."
        flash(message)
        return render_template("btc_build.html")
    if global_param_list["btc_data"]["first"] is True:
        global_param_list["btc_data"]["first"] = False
        return render_template("btc_backtest.html", time="first")
    flash("The backtest will base on {} algorithm.".format(global_param_list["btc_data"]["algorithm"]))
    data = global_param_list["btc_data"]["signal_data"]
