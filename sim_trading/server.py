...
def populate_intraday_order_map(symbols, intraday_data_table, market_periods):
    ...
    for symbol in symbols:
        ...
        select_st = "SELECT * FROM " + intraday_data_table + " WHERE symbol = \'" + symbol + "\';"
                ...
                if msg_type == PacketTypes.CONNECTION_REQ.value:
                    ...
                    if msg_data["Type"] == "Lmt":
                        ...
                        while ((server_config.order_table["Symbol"] == msg_data["Symbol"]) &
                            ...
                            (server_config.order_table["Status"] != "Filled") & (server_config.order_table["Status"] != "Open Trade" ) & \
...
def populate_order_table(symbols, start, end):
    ...
    for index in range(0, max_number_orders-1, 2):
        for i, stock_data in result_df.iterrows():
            ...
            buy_price = open_price - (index+1) * price_scale * (random.randint(1, server_config.price_unit)/server_config.price_unit)
            buy_price = float("{:.2f}".format(buy_price))
            qty = float(list_of_qty_map[stock_data['symbol']][index])
            
            server_config.order_index += 1
            order_qty = int(qty*daily_volume*(random.randint(1, server_config.price_unit)/server_config.price_unit))
            if index == 0:
                server_config.order_table.loc[server_config.order_index] = ['Srv_' + server_config.market_period + '_' + str(server_config.order_index),
            ...
            sell_price = open_price + (index+1) * price_scale * (random.randint(1, server_config.price_unit)/server_config.price_unit)
        ...
        if len(server_config.order_table) != 0 and server_config.market_status == 'Open':
            for i in range(len(symbols)):
                ...
                try:
                    ...
                    ### BUY logic
                    if ((server_config.order_table['Symbol'] == symbol) & (server_config.order_table['Side'] == 'Buy')).any():
                        ...
                        new_buy_qty = server_config.intraday_order_map[server_config.market_period][i].iloc[0]['volume']/2
                        ...
                        print(server_config.order_table.loc[server_config.order_index], file=server_config.server_output)
                        
                    #### Sell
                    if ((server_config.order_table['Symbol'] == symbol) & (server_config.order_table['Side'] == 'Sell')).any():
                        ...
                        new_sell_qty = server_config.intraday_order_map[server_config.market_period][i].iloc[0]['volume']/2
                    ...
                    server_config.intraday_order_map[server_config.market_period][i].drop(server_config.intraday_order_map[server_config.market_period][i].index[0] ,inplace=True)
                        
                    while ((server_config.order_table['Symbol'] == symbol) & (server_config.order_table['Qty'] != 0)).any():
                        buy_mask = (server_config.order_table['Symbol'] == symbol) & (server_config.order_table['Qty'] != 0) & (server_config.order_table['Side'] == 'Buy')
                        sell_mask = (server_config.order_table['Symbol'] == symbol) & (server_config.order_table['Qty'] != 0) & (server_config.order_table['Side'] == 'Sell')
                    ...
                    if server_config.mutex.locked() == True:
...
def close_trades(symbols):
    for symbol in symbols:
        ...
        if ((server_config.order_table['Symbol'] == symbol) & (server_config.order_table['Side'] == side)).any():
            mask = (server_config.order_table['Symbol'] == symbol) & (server_config.order_table['Side'] == side) & (server_config.order_table['Qty'] != 0)
            if side == 'Buy':      
...
def set_market_status(scheduler, time_in_seconds):
    ...
    for day in range(server_config.total_market_days):
        scheduler.enter((server_config.market_close_time+server_config.market_open_time+server_config.market_pending_close_time)*day+1,1,
